...
document.addEventListener('DOMContentLoaded', function () {
    const cartItemsContainer = document.querySelector(".cart .cart-items-container tbody");

    // Kiểm tra số lượng sản phẩm trong giỏ hàng
    const itemCount = cartItemsContainer.querySelectorAll("tr").length;

    // Nếu có ít nhất hai sản phẩm, thêm lớp CSS cho phần tử chứa sản phẩm
    if (itemCount >= 2) {
...
// Hàm sao chép nội dung từ một phần tử và gán nó cho các phần tử khác
function copyContent() {
    var content = document.getElementById("customContent").innerHTML;
    document.getElementById("customContent2").innerHTML = content;
    document.getElementById("customContent3").innerHTML = content;
}

// Gọi hàm sao chép khi cần thiết
copyContent();
